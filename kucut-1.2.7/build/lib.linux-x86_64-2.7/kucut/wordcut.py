...
try:
	...
	print 'AIMA dose not already been installed'
	sys.exit(1)
...
class Segmentation:
	...
	def IsSymbol(c):
		if len(c) > 1:
			return 0
		if string.find('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ\',:?.*&^%$#@!+_-><{}[]()/\\\Â“Â”"æ|Ï',c) > -1:
			return 1
		return 0
	...
	def IsNumber(number):
		if len(number) > 1:
			for d in number:
				if string.find('0123456789ñòóôõö÷øùð,.',d) == -1:
					return 0
			return 1
		if string.find('0123456789ñòóôõö÷øùð',number) > -1:
			return 1
		return 0
	...
	def divideAllCompleteList(self,clist):
		...
		for i in range(len(clist)):
			if i > 0 and i < len(clist)-1:
				begin,end = clist[i][1]
				next_begin,next_end = clist[i+1][1]
				prev_begin,prev_end = clist[i-1][1]
				if end <= next_begin and prev_end <= begin: # Check overlap
					tmpList.append(i)
		#Remove adjacent position
		i = 0
		tmpList.append(len(clist))
		...
		begin = 0
		for p in tmpList:
			start = [[x[1],x[0]] for x in clist[begin:p]]
			start.sort()
			end = [[[x[1][1],x[1][0]],x[0]] for x in clist[begin:p]]
			end.sort()
			result.append(self.line[start[0][0][0]:end[-1][0][0]])
			begin = p
	...
	def filterResult(this,result,punc=''):
		...
		for r in result:
			...
			if cost > max :
				...
				maxlist = []
				maxlist.append(r)
			elif cost == max and maxlist.count(r) == 0:
				maxlist.append(r)
	...
	def preMergeUnknownWord(self,results):
		...
		for result in results:
			...
			result = token[0] 
			while i < len(token):
				if not self.lexiconDict.contains(token[i-1]) and not self.lexiconDict.contains(token[i]):
					result += token[i]
				else:
					result += ' ' + token[i]
				i+=1
